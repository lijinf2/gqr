...
class PCAMD: public Hasher<DATATYPE>
{
public:

    typedef typename Hasher<DATATYPE>::BIDTYPE BIDTYPE;

    PCAMD() : Hasher<DATATYPE>() {};
...
vector<float> lshbox::PCAMD<DATATYPE>::getHashFloats(unsigned k, const DATATYPE *domin)
{
    // zero-centered first
    ...
    for (unsigned i = 0; i != domin_pc.size(); ++i)
    {
        ...
        domin_pc[i] -= threshold[i];
    }
    return domin_pc;
}

template<typename DATATYPE>
vector<bool> lshbox::PCAMD<DATATYPE>::getHashBits(unsigned k, const DATATYPE *domin)
{
...
template<typename DATATYPE>
void lshbox::PCAMD<DATATYPE>::loadModel(const string& modelFile, const string& baseBitsFile) {
    string line;
    ...
    this->threshold.resize(tableDim);
    getline(modelFin, line);
    istringstream thresIss(line);
    for (int i = 0; i < this->threshold.size(); ++i) {
        thresIss >> this->threshold[i];
    }
