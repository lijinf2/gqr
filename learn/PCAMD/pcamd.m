...
for j =1:nHashTable                
    ...
    fprintf(modelFid,'%f ',model.med);
    fprintf(modelFid,'\n');
   
    trainFloats = trainset * model.pc;
    trainB = (trainFloats - repmat(model.med, size(trainFloats, 1), 1) ) > 0; 
    % save base codes 
    ...
    testFloats = testset * model.pc;
    testB = (testFloats - repmat(model.med, size(testFloats, 1), 1) ) > 0;
    % save query codes
